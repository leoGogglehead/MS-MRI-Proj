%%
% This script performs evaluates tree bagger on  voxel-level on prediction made
...
count = 0;
    ...
    for i = 1 : 10
        ...
        model = TreeBagger(30,train(:,1:3),train(:,4), 'method', 'regression');
        ...
        pihat = predict(model, test(:,1:3));
        
        [x,y,T,AUC(i)] = perfcurve(test(:,4),pihat, 1);
        
        [uniqX, ind] = unique(x);
        Y = interp1(uniqX, y(ind), sp);
        yVal = yVal + Y;
